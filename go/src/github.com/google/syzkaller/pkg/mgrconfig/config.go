...
type Config struct {
	...
	FileDescriptor string `json:"file_descriptor"`

	// Should be equal to number of snapshots
	SnapShotNum int `json:"snapshot_num"`
	// Used with debug mode, debug the selected snapshot (1-indexed)
	SnapShotDebug int `json:"snapshot_debug"`
	// Snapshot Device: nvidia, nvidia1650, ... , amdgpu or mali
	SnapShotDevice     string `json:"snapshot_device"`
	SnapShotExperiment bool   `json:"snapshot_experiment"`
	...
	// Use VirtDev KCOV coverage (default: false).
	VirtCover bool `json:"vcover"`
	// Checkpoint only once
	RootChkptOnly bool `json:"rootchkptonly"`
	...
	SyzFuzzerBin   string `json:"syz_fuzzer"`
