...
func (ctx *context) genCall() *prog.Call {
	...
	if syzCall.Meta.Name == "mmap$gpu" {
		(syzCall.Args[0]).(*prog.PointerArg).TraceAddress = (uint64)(straceCall.Ret)
	}
	return syzCall
}
...
func (ctx *context) genArgs(syzType prog.Type, traceArg parser.IrType) prog.Arg {
	...
	log.Logf(0, "parsing arg of syz type: %s, ir type: %#v", syzType.Name(), traceArg)
...
func (ctx *context) genVma(syzType *prog.VmaType, traceType parser.IrType) prog.Arg {
	npages := uint64(1)
	if syzType.RangeBegin != 0 || syzType.RangeEnd != 0 {
		npages = syzType.RangeEnd
	}
	ret := prog.MakeVmaPointerArg(syzType, ctx.builder.AllocateVMA(npages), npages)
	ret.TraceAddress = (uint64)(traceType.(parser.Constant).Val())
	return ret
}
...
func (ctx *context) genBuffer(syzType *prog.BufferType, traceType parser.IrType) prog.Arg {
	...
	case *parser.GroupType:
		for _, elem := range a.Elems {
			bufVal = append(bufVal, []byte(elem.String()[2:])...)
		}
	default:
		// log.Fatalf("unsupported type for buffer: %#v", traceType)
		fmt.Printf("unsupported type for buffer: %#v", traceType)
...
func (ctx *context) genResource(syzType *prog.ResourceType, traceType parser.IrType) prog.Arg {
	...
	case parser.Constant:
		...
		if arg := ctx.returnCache.get(syzType, traceType); arg != nil {
			res := prog.MakeResultArg(syzType, arg.(*prog.ResultArg), val)
			return res
		}
