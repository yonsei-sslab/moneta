...
type Manager struct {
	...
	// moneta
	fileDescriptorDir string
	fileDescriptorDB  [][]int
	matchSnapPoint    map[int]int
	FdCount           [][]int
}
...
func RunManager(cfg *mgrconfig.Config, target *prog.Target, sysTarget *targets.Target, syscalls []int) {
	...
	// MakeFileDescriptorDB(cfg.FileDescriptor)
	filedescriptordb := MakeFileDescriptorDB(cfg.FileDescriptor)
	...
	mgr := &Manager{
		...
		fileDescriptorDB: filedescriptordb,
	}
	...
	if cfg.Type != "none" {
		var err error
		mgr.vmPool, err = vm.Create(cfg, *flagDebug)
		if err != nil {
			log.Fatalf("%v", err)
		}
	}
	...
	if cfg.SnapShotNum > 0 {
		mgr.matchSnapPoint = mgr.vmPool.GetmatchSnapPoint()
	}
...
func (mgr *Manager) loadCorpus() {
	...
	idx := 0
	mgr.FdCount = make([][]int, mgr.cfg.SnapShotNum+1)
	...
	if mgr.cfg.SnapShotNum > 0 {
		SnapShotPerInst := math.Ceil(float64(mgr.vmPool.Count() / mgr.cfg.SnapShotNum))
		tmp := make([]rpctype.RPCCandidate, 0)
		tmp = append(tmp, mgr.candidates...)

		if SnapShotPerInst > 1 {
			for i := 0; i < int(SnapShotPerInst-1); i++ {
				mgr.candidates = append(mgr.candidates, tmp...)
			}
		}
	}
...
func (mgr *Manager) runInstance(index int) (*Crash, error) {
	...
	// 	mgr.cfg.Cover, *flagDebug, false, false, int64(index))
	cmd := ""
...
monitor:
	...
	// Check if we can restore on crash
	select {
	case <-mgr.vmStop:
		mgr.vmStop <- true
		return crash, nil
	...
	if ctlpipe, stpipe := inst.GetPipes(); ctlpipe != nil && stpipe != nil {
		var buf [4]byte
		n, err := io.ReadFull(stpipe, buf[:])
		if err != nil {
			log.Logf(0, "mgr pipe (st) read failed on crash")
			return crash, nil
		}
		if n != 4 {
			log.Fatalf("mgr pipe (st) returned more than 4 bytes")
		}
		log.Logf(0, "vm-%v: crashed.", index)
		// moneta for debugging
		fmt.Printf("vm-%v: crashed.", index)

		if bytes.Equal(buf[:], []byte{0x0d, 0xf0, 0xad, 0x0b}) {
			needRepro := mgr.saveCrash(crash)
			if needRepro {
				log.Logf(0, "repro not supported yet in PeriScope mode")
			}

			log.Logf(0, "vm-%v: restoring...", index)
			b := []byte{0x0d, 0xf0, 0xad, 0xde}
			n, err := ctlpipe.Write(b)
			if err != nil {
				log.Logf(0, "mgr pipe (ctl) write failed on restoring")
				return crash, nil
			}
			if n != 4 {
				log.Fatalf("mgr pipe (ctl) returned more than 4 bytes")
			}
			goto monitor
		}

		log.Logf(0, "vm-%v: rebooting...", index)
		stpipe.Close()
		ctlpipe.Close()
	}
	return crash, nil
...
func (mgr *Manager) saveCrash(crash *Crash) bool {
	...
	snap_info := []byte(fmt.Sprintf("vm-%v: snapshot point-%v\n", crash.vmIndex, mgr.matchSnapPoint[crash.vmIndex]))
	crash.Output = append(snap_info, crash.Output...)
	...
	for i := 0; i < 100; i++ {
		...
		if err != nil {
			oldestI = i
			if i == 0 {
				// Save the first crash separately so that it is never overwritten
				osutil.WriteFile(filepath.Join(dir, "log"), crash.Output)
				if len(mgr.cfg.Tag) > 0 {
					osutil.WriteFile(filepath.Join(dir, "tag"), []byte(mgr.cfg.Tag))
				}
				if len(crash.Report.Report) > 0 {
					osutil.WriteFile(filepath.Join(dir, "report"), crash.Report.Report)
				}
...
func (mgr *Manager) newInput(inp rpctype.RPCInput, sign signal.Signal) {
	...
	if old, ok := mgr.corpus[sig]; ok {
		...
		mgr.corpus[sig] = inp
		mgr.corpusDB.Save(sig, inp.Prog, 0, inp.SnapPoint)
...
func (mgr *Manager) snapCandidateBatch(index int) []rpctype.RPCCandidate {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	var res []rpctype.RPCCandidate

	snapPoint := mgr.matchSnapPoint[index]

	for i, candidate := range mgr.candidates {
		if snapPoint == candidate.SnapPoint {
			log.Logf(0, "DBG find matched! %v %v %v\n", candidate.SnapPoint, i, snapPoint)
			res = append(res, candidate)
			mgr.candidates = append(mgr.candidates[0:i], mgr.candidates[i+1:]...)
			break
		}
	}
...
func MakeFileDescriptorDB(dir string) [][]int {
	files, err := ioutil.ReadDir(dir)
	if err != nil {
		fmt.Errorf("Failed to read filedescriptor dir\n")
		return nil
	}

	// var filedescriptordb [][]int
	filedescriptordb := make([][]int, len(files))

	for i, file := range files {
		f, err := os.Open(filepath.Join(dir, file.Name()))

		if err != nil {
			fmt.Errorf("Failed to read filedescriptor file\n")
			return nil
		}
		defer f.Close()
		...
		for scanner.Scan() {
			fd, _ := strconv.Atoi(scanner.Text())
			filedescriptordb[i] = append(filedescriptordb[i], fd)
		}
