...
ioctl_decode(struct tcb *tcp)
{
	...
	const unsigned int cmd = _IOC_NR(code);
	...
	case 'd':
		if (cmd >= 0x40 && cmd <= 0x55)
			return amdgpu_ioctl(tcp, code, arg);
		break;
	case 'f':
...
#endif /* !ALPHA */
	case 0x80:
		return mali_ioctl(tcp, code, arg);
	case 0x89:
		return sock_ioctl(tcp, code, arg);
	case 0x94:
...
SYS_FUNC(ioctl)
{
	...
	if (entering(tcp)) {
		...
		unsigned int type = _IOC_TYPE((unsigned int)tcp->u_arg[1]);
		unsigned int cmd = _IOC_NR((unsigned int)tcp->u_arg[1]);

		if ((type == 0x46) || \
			(type == 0x80) || \
			(type == 'd' && (cmd >= 0x40 && cmd <= 0x55))){
			handle_moneta_open(tcp);
			// syz_ioctl_nvidia(fd fd_gpudev, topbyte const[0xc0], count len[arg], magic const[0x46], cmd flags[nvidia_cmd], arg buffer[in])
			PRINT_VAL_X((int8_t)((tcp->u_arg[1] & 0xff000000) >> 24));
			tprint_arg_next();
			PRINT_VAL_X((int8_t)((tcp->u_arg[1] & 0x00ff0000) >> 16));
			tprint_arg_next();
			PRINT_VAL_X((int8_t)((tcp->u_arg[1] & 0x0000ff00) >> 8));
			tprint_arg_next();
			PRINT_VAL_X((int8_t)((tcp->u_arg[1] & 0x000000ff)));
		}
		else if (xlat_verbosity != XLAT_STYLE_ABBREV)
