...
bool write_list_to_cpustate(ARMCPU *cpu)
{
    ...
    for (i = 0; i < cpu->cpreg_array_len; i++) {
         ...
         */
		// copy only when register is not CONST
		if (!(ri->type & ARM_CP_CONST))
			write_raw_cp_reg(&cpu->env, ri, v);
        if (!(ri->type & ARM_CP_CONST) && read_raw_cp_reg(&cpu->env, ri) != v) {
#ifdef CONFIG_DEBUG_SAVEVM
      qemu_log("##### Register value is different!! #####");
			// stderr
      qemu_log("type: 0x%x %s reg: 0x%lx v: 0x%lx\n",
				ri->type, ri->name, read_raw_cp_reg(&cpu->env, ri), v);
      // stdout
      printf("type: 0x%x %s reg: 0x%lx v: 0x%lx\n",
				ri->type, ri->name, read_raw_cp_reg(&cpu->env, ri), v);
#endif
            ok = false;
        }
...
#ifdef CONFIG_DEBUG_SAVEVM
        qemu_log("%s: cpreg[%3d]\t %-25s cpreg_indexes 0x%016lx\t regidx 0x%08x\n",
                __func__, cpu->cpreg_array_len, ri->name, cpu->cpreg_indexes[cpu->cpreg_array_len], *(uint32_t *)key);
#endif
...
#ifdef CONFIG_DEBUG_SAVEVM
    qemu_log("%s: %-25s key 0x%08x type 0x%x\n",
            __func__, r2->name, *key, r2->type);
#endif
