...
decode_open(struct tcb *tcp, int offset)
{
	...
	if (moneta_exec_tracee) {
		#ifdef __x86_64
		if ((tcp->u_arg[offset + 1] & ~O_CLOEXEC) &&
				ptrace(PTRACE_POKEUSER, tcp->pid, (13 - offset) * sizeof(unsigned long), tcp->u_arg[offset + 1] & ~O_CLOEXEC) == -1)
			error_msg_and_die("Could not overwrite flags for openat2");
		...
		if (ptrace(PTRACE_GETREGSET, tcp->pid, (void*)NT_PRSTATUS, (void*)&registers) == -1)
		{
			printf("%d\n", errno);
			error_msg_and_die("Something went horribly wrong...");
		}
		{
			printf("moneta: open_decode: read registers (%ld bytes)\n", registers.iov_len);
		}
		regs.regs[2] = regs.regs[2] & ~O_CLOEXEC;
		if (ptrace(PTRACE_SETREGSET, tcp->pid, (void*)NT_PRSTATUS, (void*)&registers) < 0)
		{
...
print_open_how(struct tcb *tcp, kernel_ulong_t addr, kernel_ulong_t size)
{
	...
	if (moneta_exec_tracee) {
		...
		{
			how.flags &= ~O_CLOEXEC;
			upoken(tcp, addr, MIN(size, sizeof(how)), &how);
		}
	}
