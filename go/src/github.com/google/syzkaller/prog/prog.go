...
func (p *Prog) SetReturnNum() {
	fdargs := make(map[*ResultArg]bool)

	for _, c := range p.Calls {
		if c.Ret != nil {
			fdargs[c.Ret] = true
			c.Ret.Num = len(fdargs)
		}
		for _, arg := range c.Args {
			setReturnNum(&arg, fdargs)
		}
	}
}

func setReturnNum(arg *Arg, fdargs map[*ResultArg]bool) {
	switch a := (*arg).(type) {
	case *ConstArg:
	case *PointerArg:
		if a.Res != nil {
			setReturnNum(&a.Res, fdargs)
		}
	case *DataArg:
	case *GroupArg:
		for _, arg2 := range a.Inner {
			setReturnNum(&arg2, fdargs)
		}
	case *UnionArg:
		setReturnNum(&a.Option, fdargs)
	case *ResultArg:
		if a.Res != nil {
			if fdargs[a.Res] == false {
				fdargs[a.Res] = true
				a.Res.Num = len(fdargs)
			}
		}
