...
func pack(dir, file string, target *prog.Target, version uint64) {
	...
	for _, file := range files {
		...
		var seq uint64
		key := file.Name()
		// moneta: Maybe we don't need it now...
		...
		var snapPoint uint64
		if parts := strings.Split(file.Name(), "."); len(parts) == 2 {
			var err error
			if snapPoint, err = strconv.ParseUint(parts[1], 10, 64); err != nil {
				fmt.Errorf("failed to parse snapPoint: %v", err)
				return
			}
...
func unpack(file, dir string) {
	...
	for key, rec := range db.Records {
		...
		if rec.SnapPoint != 0 {
			fname += fmt.Sprintf(".%v", rec.SnapPoint)
		}
