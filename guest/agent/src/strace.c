...
\n\
Moneta:\n\
  --moneta-n N  System call frequency for snapshotting\n\
  --moneta-t T  Timeout after which post snapshot tracing should stop\n\
  --moneta-e    Exec tracee instead of tracer for fuzzing\n\
  --moneta-stdout\n\
                 Reopen stdout fd in tracee before execing it.\n\
  --moneta-one-log\n\
                 Only log output after gpu process has been identified\n\
"
...
after_successful_attach(struct tcb *tcp, const unsigned int flags)
{
	...
	if (output_separately && !moneta_one_log) {
		char name[PATH_MAX];
...
# error Bug in DEFAULT_QUAL_FLAGS
	...
	enum {
		...
		GETOPT_MONETA_E,
		GETOPT_MONETA_S,
		GETOPT_MONETA_STDOUT,
		GETOPT_MONETA_ONE_LOG,
		GETOPT_MONETA_BUG,
		GETOPT_MONETA_TARGET_DRIVER,
	};
...
#endif
		{ "moneta-n",	required_argument, 0, GETOPT_MONETA_N },
		{ "moneta-t",	required_argument, 0, GETOPT_MONETA_T },
		{ "moneta-e",	no_argument,       0, GETOPT_MONETA_E },
		{ "moneta-s",	required_argument, 0, GETOPT_MONETA_S },
		{ "moneta-stdout",	no_argument,   0, GETOPT_MONETA_STDOUT },
		{ "moneta-bug",	required_argument, 0, GETOPT_MONETA_BUG },
		{ "moneta-driver",	required_argument, 0, GETOPT_MONETA_TARGET_DRIVER },
		{ "moneta-one-log", no_argument,  0, GETOPT_MONETA_ONE_LOG },
	...
	while ((c = getopt_long(argc, argv, optstring, longopts, &lopt_idx)) != EOF) {
		...
		case GETOPT_QUAL_DECODE_PID:
			...
			break;
		case GETOPT_MONETA_N:
			moneta_syscall_threshold = string_to_uint(optarg);
			break;
		case GETOPT_MONETA_T:
			moneta_timeout = string_to_uint(optarg);
			break;
		case GETOPT_MONETA_E:
			moneta_exec_tracee = 1;
			break;
		case GETOPT_MONETA_STDOUT:
			moneta_reopen_stdout = 1;
			break;
		case GETOPT_MONETA_BUG:
			moneta_bug_id = string_to_uint(optarg);
			break;
		case GETOPT_MONETA_TARGET_DRIVER:
			if (!strcmp(optarg, "nvidia"))
					moneta_target_driver = 1;
			else if (!strcmp(optarg, "mali"))
				moneta_target_driver = 2;
			else if (!strcmp(optarg, "amdgpu"))
				moneta_target_driver = 3;
			else
				moneta_target_driver = 0;
			break;
		case GETOPT_MONETA_S:
			moneta_snapnum = string_to_uint(optarg);
			break;
		case GETOPT_MONETA_ONE_LOG:
			moneta_one_log = 1;
			break;
		default:
...
trace_syscall(struct tcb *tcp, unsigned int *sig)
{
	if (entering(tcp)) {
		...
		#if 0
		if (moneta_syscall_count >= 0 && tcp->pid == moneta_gpu_pid)
...
int
kill_all_tracees(bool but_one)
{
	// readlink -f /proc/*/task/%d, tid
	int* pids = malloc(sizeof(int)*tcbtabsize);
	int pid_count = 0;
	char tmp[4096];
	char cmd[128];
	...
	for (unsigned int i = 0; i < tcbtabsize; i++)
	{
		...
		sprintf(cmd, "readlink -f /proc/*/task/%d | cut -d \"/\" -f 3", tcbtab[i]->pid);
		FILE* fp = popen(cmd, "r");

		if (!fp || feof(fp))
		{
			error_msg_and_die("Couldn't determine pid for tid %d", tcbtab[i]->pid);
			exit(-1);
			return 0;
		}
		...
		int found_pid = strtol(tmp, NULL, 10);
		pclose(fp);

		int pid_i;
		for (pid_i = 0; pid_i < pid_count; pid_i++)
			if (pids[pid_i] == found_pid)
				break;
		if (pid_i == pid_count)
		{
			printf(" > tid %d maps to pid %d\n", tcbtab[i]->pid, found_pid);
			pids[pid_count++] = found_pid;
		}
		if (tcbtab[i]->pid == moneta_gpu_pid)
			gpu_pid = found_pid;
	}

	for (int pid_i = 0; pid_i < pid_count; pid_i++)
	{
		if (!(but_one && pids[pid_i] == gpu_pid))
		{
			printf("Killing %d\n", pids[pid_i]);
			kill(pids[pid_i], SIGKILL);
		}
	}
	free(pids);
	return gpu_pid;
...
void
open_one_log(struct tcb *const tcp)
{
	tcp->outf = strace_fopen(outfname);
}

static void
moneta_delete_old_outputs(void)
{
	const char *dir_path = "/root";  // Specify the directory path here
	...
	while ((entry = readdir(dir)) != NULL) {
		...
		if (strncmp(entry->d_name, "moneta.trace", strlen("moneta.trace")) == 0 ||
			strncmp(entry->d_name, "sfd_tfd_", strlen("sfd_tfd_")) == 0) {			
			if (remove(path) == 0) {
				error_msg("Deleted: %s", path);
			} else {
				error_msg("Error deleting file: %s", path);
			}
		}
	}
...
{
	/*
		kInFd and kOutFd are hardcoded to 3 and 4 in executor.cc
	*/
	moneta_delete_old_outputs();
