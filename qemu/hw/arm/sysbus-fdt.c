...
static struct {
    ...
    { "#cooling-cells",             PROP_COPY },
    { "operating-points-v2",        PROP_IGNORE },
    { "dynamic-power-coefficient",  PROP_IGNORE },
    { "upthreshold",                PROP_IGNORE },
    { "downdifferential",           PROP_IGNORE },
    { "mali-supply",                PROP_IGNORE },
    { "mem-supply",                 PROP_IGNORE },
...
static int add_generic_fdt_node(SysBusDevice *sbdev, void *opaque)
{
    ...
    node_name = g_strdup_printf("%s/%s@%" PRIx64, parent_node,
                               vbasedev->name, mmio_base);
    printf("node_name: %s mmio_base: 0x%lx\n", node_name, mmio_base);
...
/**
 * add_fakedev_fdt_node
 *
 * Generates a Mali fake device DT node by using hard-coded properties from the host
 */
static int add_fakedev_fdt_node(SysBusDevice *sbdev, void *opaque)
{
    PlatformBusFDTData *data = opaque;
    VFIOPlatformDevice *vdev = PLATFORM_MALI(sbdev);
    const char *parent_node = data->pbus_node_name;
...
static bool mali_platform_match(SysBusDevice *sbdev,
                                const BindingEntry *entry)
{
    return true;
}
...
#define MALI_PLATFORM_BINDING(compat, add_fn) \
    {TYPE_PLATFORM_MALI, (compat), (add_fn), mali_platform_match}
...
static const BindingEntry bindings[] = {
    ...
    /* Generic DT fallback must be last */
    MALI_PLATFORM_BINDING(NULL, add_fakedev_fdt_node),
#endif
