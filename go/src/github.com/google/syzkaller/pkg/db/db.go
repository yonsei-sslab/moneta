...
func (db *DB) Save(key string, val []byte, seq uint64, snapPoint int) {
	if seq == seqDeleted {
		panic("reserved seq")
	}
	...
	db.Records[key] = Record{val, seq, snapPoint}
	db.serialize(key, val, seq, snapPoint)
	db.uncompacted++
}
...
func (db *DB) Delete(key string) {
	...
	db.serialize(key, nil, seqDeleted, 0)
	db.uncompacted++
}
...
func (db *DB) compact() error {
	...
	for key, rec := range db.Records {
		serializeRecord(buf, key, rec.Val, rec.Seq, rec.SnapPoint)
	}
...
func (db *DB) serialize(key string, val []byte, seq uint64, snapPoint int) {
	if db.pending == nil {
		db.pending = new(bytes.Buffer)
	}
	serializeRecord(db.pending, key, val, seq, snapPoint)
}
...
func serializeRecord(w *bytes.Buffer, key string, val []byte, seq uint64, snapPoint int) {
	...
	binary.Write(w, binary.LittleEndian, uint32(snapPoint))
	if len(val) == 0 {
...
func deserializeDB(r *bufio.Reader) (version uint64, records map[string]Record, uncompacted int) {
	...
	version = ver
	for {
		key, val, seq, snapPoint, err := deserializeRecord(r)
		if err == io.EOF {
			return
		}
		if err != nil {
		...
		if seq == seqDeleted {
			...
			records[key] = Record{val, seq, int(snapPoint)}
		}
	}
}
...
func deserializeRecord(r *bufio.Reader) (key string, val []byte, seq uint64, snapPoint uint32, err error) {
	var magic uint32
	if err = binary.Read(r, binary.LittleEndian, &magic); err != nil {
		return
	}
	...
	if err = binary.Read(r, binary.LittleEndian, &snapPoint); err != nil {
		return
	}
	var valLen uint32
...
func Create(filename string, version uint64, records []Record) error {
	...
	for _, rec := range records {
		db.Save(hash.String(rec.Val), rec.Val, rec.Seq, rec.SnapPoint)
	}
