...
static int cpu_post_save(void *opaque)
{
    ...
    for (int i=0; i< cpu->cpreg_array_len; i++) {
        ...
        if (!ri) {
            qemu_log("%s: reg[%d]\t %-25s cpreg_vmstate_indexes 0x%016lx\t regidx 0x%08x\t cpreg_vmstate_values 0x%lx\n",
                __func__, i, "NULL", cpu->cpreg_vmstate_indexes[i], regidx, cpu->cpreg_vmstate_values[i]);
            continue;
        }
        qemu_log("%s: reg[%d]\t %-25s cpreg_vmstate_indexes 0x%016lx\t regidx 0x%08x\t cpreg_vmstate_values 0x%lx\n",
            __func__, i, ri->name, cpu->cpreg_vmstate_indexes[i], regidx, cpu->cpreg_vmstate_values[i]);
    }
#endif
...
static int cpu_post_load(void *opaque, int version_id)
{
    ...
    // dump vmstate registers before restoring
#ifdef CONFIG_DEBUG_SAVEVM
    qemu_log("##### [DEBUG] Before restoring cp registers #####\n");
    qemu_log("########## cpreg_vmstate_array ##########\n");
    const ARMCPRegInfo *ri;
    uint32_t regidx;
    for (int i=0; i< cpu->cpreg_vmstate_array_len; i++) {
        regidx = kvm_to_cpreg_id(cpu->cpreg_vmstate_indexes[i]);

        ri = get_arm_cp_reginfo(cpu->cp_regs, regidx);
        if (!ri) {
            qemu_log("Snapshot before restoring: reg[%d]\t %-25s cpreg_vmstate_indexes 0x%016lx\t regidx 0x%08x\t cpreg_vmstate_values 0x%lx\n",
                i, "NULL", cpu->cpreg_vmstate_indexes[i], regidx, cpu->cpreg_vmstate_values[i]);
            continue;
        }
        qemu_log("Snapshot before restoring: reg[%d]\t %-25s cpreg_vmstate_indexes 0x%016lx\t regidx 0x%08x\t cpreg_vmstate_values 0x%lx\n",
            i, ri->name, cpu->cpreg_vmstate_indexes[i], regidx, cpu->cpreg_vmstate_values[i]);
    }

    qemu_log("########## cpreg_array ##########\n");
    for (int i=0; i< cpu->cpreg_array_len; i++) {
        regidx = kvm_to_cpreg_id(cpu->cpreg_indexes[i]);

        ri = get_arm_cp_reginfo(cpu->cp_regs, regidx);
        if (!ri) {
            qemu_log("Local before restoring: reg[%d]\t %-25s cpreg_indexes 0x%016lx\t regidx 0x%08x\t cpreg_values 0x%lx\n",
                i, "NULL", cpu->cpreg_indexes[i], regidx, cpu->cpreg_values[i]);
            continue;
        }
        qemu_log("Local before restoring: reg[%d]\t %-25s cpreg_indexes 0x%016lx\t regidx 0x%08x\t cpreg_values 0x%lx\n",
            i, ri->name, cpu->cpreg_indexes[i], regidx, cpu->cpreg_values[i]);
    }
#endif

	// turn off safety check of snapshot
...
#else
        if (cpu->cpreg_vmstate_indexes[v] < cpu->cpreg_indexes[i]) {
            /* register in their list but not ours: skip it */
			while (cpu->cpreg_vmstate_indexes[v] < cpu->cpreg_indexes[i]) {
				v++;
			}
        }
        if (cpu->cpreg_vmstate_indexes[v] == cpu->cpreg_indexes[i]){
...
#ifdef CONFIG_DEBUG_SAVEVM
    qemu_log("##### [DEBUG] After restoring cp registers #####\n");
    for (int i=0; i< cpu->cpreg_array_len; i++) {
        uint32_t regidx = kvm_to_cpreg_id(cpu->cpreg_indexes[i]);
        const ARMCPRegInfo *ri;
        ...
        ri = get_arm_cp_reginfo(cpu->cp_regs, regidx);
        if (!ri) {
            qemu_log("Local after restoring: reg[%3d]\t %-25s cpreg_indexes 0x%016lx\t regidx 0x%08x\t cpreg_values 0x%lx\n",
                i, "NULL", cpu->cpreg_indexes[i], regidx, cpu->cpreg_values[i]);
            continue;
        }
        qemu_log("Local after restoring: reg[%3d]\t %-25s cpreg_indexes 0x%016lx\t regidx 0x%08x\t cpreg_values 0x%lx\n",
