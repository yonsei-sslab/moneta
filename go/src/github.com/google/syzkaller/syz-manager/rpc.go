...
type RPCManagerView interface {
	...
	snapCandidateBatch(index int) []rpctype.RPCCandidate
	rotateCorpus() bool
}
...
func startRPCServer(mgr *Manager) (int, error) {
	serv := &RPCServer{
		...
		fileDescriptorDB: mgr.fileDescriptorDB,
		matchSnapPoint:   &mgr.matchSnapPoint,
		fdCount:          &mgr.FdCount,
	}
	serv.batchSize = 5
...
func (serv *RPCServer) Poll(a *rpctype.PollArgs, r *rpctype.PollRes) error {
	...
	if a.NeedCandidates {
		...
		r.Candidates = serv.mgr.snapCandidateBatch(index)
	}
...
func (serv *RPCServer) GetFileDescriptor(a *rpctype.FiledescriptorArgs, r *rpctype.FiledescriptorRes) error {
	serv.mu.Lock()
	defer serv.mu.Unlock()

	if len(serv.fileDescriptorDB) != 0 {
		r.FileDescriptorDB = serv.fileDescriptorDB[0]
	}

	return nil
}

func (serv *RPCServer) GetfdCount(a *rpctype.FdCountArgs, r *rpctype.FdCountRes) error {
	serv.mu.Lock()
	defer serv.mu.Unlock()

	if len(*serv.fdCount) != 0 {
		r.SnapPoint = (*serv.matchSnapPoint)[a.Index]
		r.FdCount = (*serv.fdCount)[(*serv.matchSnapPoint)[a.Index]]
		log.Logf(0, "Passed fd info to fuzzer: vm-%v snapPoint: %v", a.Index, (*serv.matchSnapPoint)[a.Index])
	}
