...
static void printk(const char* fmt, ...)
{
	...
	if (kmsg_fd > 0) {
		...
		va_end(args);
		printf("printk fd=%d msg=%s\n", kmsg_fd, msg);
		if (write(kmsg_fd, msg, strlen(msg) + 1) != ssize_t(strlen(msg) + 1)) {
			printf("printk write failed\n");
		}
		close(kmsg_fd);
	} else {
...
#if SYZ_EXECUTOR || __NR_syz_get_snapfd
static long syz_get_snapfd()
{
	return snapfd++; // DONE
}
#endif

#if SYZ_EXECUTOR || __NR_syz_ioctl_nvidia || __NR_syz_ioctl_nvidia$xfer

// syz_ioctl_nvidia$xfer(fd fd_gpudev, topbyte const[0xc0], count len[arg], magic const[0x46], cmd flags[NV_ESC_IOCTL_XFER_CMD], arg ptr[in, nv_ioctl_xfer])
#define NV_ABSOLUTE_MAX_IOCTL_SIZE 16384
#define NV_ESC_IOCTL_XFER_CMD 211
struct nv_ioctl_xfer {
	__u32 cmd;
	__u32 size;
	__u8 data[NV_ABSOLUTE_MAX_IOCTL_SIZE];
};

static long syz_ioctl_nvidia(int fd, __u8 topbyte, __u8 count, __u8 magic, __u8 cmd, unsigned char* buffer)
{
	__u32 ioctl_cmd =
	    topbyte << 24 |
	    count << 16 |
	...
	debug_dump_data((const char*)buffer, 0xb8);

	if (cmd == NV_ESC_IOCTL_XFER_CMD) {
		return ioctl(fd, ioctl_cmd, (struct nv_ioctl_xfer*)buffer);

	} else {
		return ioctl(fd, ioctl_cmd, buffer);
	}
}
#endif

#if SYZ_EXECUTOR || __NR_syz_ioctl_mali

static long syz_ioctl_mali(int fd, __u8 topbyte, __u8 count, __u8 magic, __u8 cmd, unsigned char* buffer)
{
	__u32 ioctl_cmd =
	    topbyte << 24 |
	    count << 16 |
...
#if SYZ_EXECUTOR || __NR_syz_ioctl_amdgpu

static long syz_ioctl_amdgpu(int fd, __u8 topbyte, __u8 count, __u8 magic, __u8 cmd, unsigned char* buffer)
{
	__u32 ioctl_cmd =
	    topbyte << 24 |
	    count << 16 |
...
#ifdef __aarch64__
__attribute__((unused)) static void reset_net_namespace(void)
#else
...
#ifdef __aarch64__
__attribute__((unused)) static void reset_loop()
#else
