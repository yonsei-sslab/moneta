...
syscall_entering_decode(struct tcb *tcp)
{
	...
	if (res != 1 || (res = get_syscall_args(tcp)) != 1) {
		printleader(tcp);
		if (!strcmp(tcp_sysent(tcp)->sys_name, "ioctl")) {
			unsigned int type = _IOC_TYPE((unsigned int)tcp->u_arg[1]);
			unsigned int cmd = _IOC_NR((unsigned int)tcp->u_arg[1]);

			if (type == 0x46)
				tprints_arg_begin("syz_ioctl_nvidia");
			else if (type == 0x80)
				tprints_arg_begin("syz_ioctl_mali");
			else if (type == 'd' && (cmd >= 0x40 && cmd <= 0x55))
				tprints_arg_begin("syz_ioctl_amdgpu");
			else
